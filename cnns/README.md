...
| 06 |	Enhancement & Spatial Filtering (classical CV, non-curriculum) |	User-uploaded image via Colab files.upload() (.jpg/.jpeg/.png)	| Side-by-side comparison of Gaussian blur vs. three sharpening methods — PIL's built-in SHARPEN, plus two custom cv2.filter2D implementations ("Volvo8" and "General") using an identical 3×3 Laplacian-based kernel [[0,-1,0],[-1,5,-1],[0,-1,0]], deliberately included twice to demonstrate that sharpening behavior is defined by the kernel weights, not the function name |
